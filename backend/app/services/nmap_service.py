...
from app.models.schemas import (
    NmapScanResult, NmapScanType, NmapTarget,
    DiscoveredHost, ScanStatus,
)
...
class NmapService:
    ...
    async def scan(self, target: NmapTarget) -> NmapScanResult:
        ...
        result = await process_manager.run(cmd, timeout=target.timeout)
    ...
    async def discover_network(self, cidr: str) -> NmapScanResult:
        ...
        return await self.scan(NmapTarget(
            target=cidr,
            scan_type=NmapScanType.QUICK,
            timeout=120,
        ))

    async def deep_scan_host(self, ip: str) -> NmapScanResult:
        """Full port + service + OS scan on a single host."""
        return await self.scan(NmapTarget(
            target=ip,
            scan_type=NmapScanType.FULL,
            timeout=600,
        ))

    async def vuln_scan(self, ip: str) -> NmapScanResult:
        """Run nmap vuln scripts against a host."""
        return await self.scan(NmapTarget(
            target=ip,
            scan_type=NmapScanType.VULN,
            timeout=600,
        ))

    async def probe_router(self, ip: str = "192.168.0.1") -> dict:
        """
        Specific probe for router CTF objective:
        - Check common ports (80, 443, 22, 23, 8080, 8443)
        - Identify services and versions
        - Check for known default credentials pages
        """
        scan = await self.scan(NmapTarget(
            target=ip,
            scan_type=NmapScanType.SERVICE,
