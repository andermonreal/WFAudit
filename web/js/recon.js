...
async function wifiscan(){
  ...
  loadScHist();
  if(ST.activeScan&&ST.activeScan.id)resumeScan();}
// Persiste el escaneo en curso para poder reanudar la vista (barra + resultados)
// al volver a la pestaña o recargar la página; el backend sigue escaneando en 2º plano.
function _setActiveScan(v){ST.activeScan=v||null;try{v?localStorage.setItem('activeScan',JSON.stringify(v)):localStorage.removeItem('activeScan');}catch(e){}}
async function resumeScan(){
  const a=ST.activeScan;if(!a||!a.id)return;
  const el=Math.floor((Date.now()-(a.started||Date.now()))/1000);
  try{
    const sc=await A.getScan(a.id);_scan=sc;
    if(sc.status==='running'){
      const b=document.getElementById('ws-btn');if(b)b.disabled=true;
      updateScanSt(sc,Math.min(el,a.dur),a.dur,'running');
      startScanPoll(a.id,a.dur,el);          // reanuda el polling desde el tiempo transcurrido
    }else{                                    // terminó mientras estábamos fuera
      ST.lastScan=sc;try{localStorage.setItem('lastScan',JSON.stringify(sc));}catch(e){}
      updateScanSt(sc,a.dur,a.dur,sc.status);renderScanR(sc);loadScHist();_setActiveScan(null);
    }
  }catch(e){_setActiveScan(null);}
}
...
async function startScan(){const iface=document.getElementById('ws-if')?.value,band=document.getElementById('ws-band')?.value||'bg',dur=parseInt(document.getElementById('ws-dur')?.value)||30,ch=document.getElementById('ws-ch')?.value,bssid=document.getElementById('ws-bssid')?.value?.trim(),essid=document.getElementById('ws-essid')?.value?.trim();if(!iface){toast('Selecciona interfaz','warn');return;}try{document.getElementById('ws-btn').disabled=true;const sc=await A.startScan({interface:iface,band,duration:dur,channel:ch?parseInt(ch):null,target_bssid:bssid||null,target_essid:essid||null});_scan=sc;_setActiveScan({id:sc.id,dur,started:Date.now()});toast('Escaneo iniciado','success');startScanPoll(sc.id,dur);}catch(e){toast(e.message,'error');const b=document.getElementById('ws-btn');if(b)b.disabled=false;}}
function startScanPoll(id,dur,el0=0){let el=el0;if(_scanPoll)clearInterval(_scanPoll);if(!el0)updateScanSt(null,0,dur,'running');_scanPoll=setInterval(async()=>{el=Math.min(el+2,dur);try{const sc=await A.getScan(id);_scan=sc;updateScanSt(sc,el,dur,sc.status);if(sc.status!=='running'){clearInterval(_scanPoll);_scanPoll=null;ST.lastScan=sc;localStorage.setItem('lastScan',JSON.stringify(sc));_setActiveScan(null);renderScanR(sc);loadScHist();const b=document.getElementById('ws-btn');if(b)b.disabled=false;}}catch{}},2000);}
...
// Salta a Avanzado → pestaña WPS con el AP objetivo precargado
function goWPS(bssid,essid,ch){setIP({'wps-bssid':bssid,'wps-ch':ch});try{localStorage.setItem('adv-tab','4');}catch(e){}goto('advanced');}
